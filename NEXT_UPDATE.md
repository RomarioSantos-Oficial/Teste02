...
- [x] STR: manter sempre a marca/logo do carro na coluna `MAR` e mover a bandeira de chegada para a coluna automática `STS`, que mostra chegada, garagem, pit, DNF, DQ, bandeira amarela e punições, sem exibir `INV`.
...
- [x] STR: confirmar individualmente a volta à frente no GAP, sem atribuir `-1L` aos outros carros quando somente o líder ultrapassou o jogador.
- [x] Flags: manter a bandeira amarela/FCY oficial visível mesmo quando não for possível identificar o carro causador no alcance do radar.
