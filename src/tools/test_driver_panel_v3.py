...
class TestWindow(QWidget):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Teste Telemetry V3")
        self.resize(360, 150)
        ...
        self.editor = DriverPanelEditor(
            self.manager.config_data["widgets"]["driver_panel"],
            self,
        )
        self.editor.config_changed.connect(
            lambda cfg: self.manager.update_widget_config(
                "driver_panel", cfg
            )
        )
        ...
        clear.clicked.connect(self.panel.clear_graph)
    ...
    def animate(self) -> None:
        ...
        self.player.throttle = max(
            0.0,
            min(1.0, 0.50 + 0.50 * math.sin(self.time_value)),
        )
        self.player.brake = max(
            0.0,
            min(1.0, 0.50 + 0.50 * math.sin(self.time_value + 3.0)),
        )
        self.player.steering = math.sin(self.time_value * 0.65)

        self.player.rpm = 2500 + self.player.throttle * 6500
        self.player.speed_kmh = 70 + self.player.throttle * 250
        self.player.gear = max(1, min(7, int(self.player.speed_kmh // 45) + 1))
