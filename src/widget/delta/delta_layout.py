...
class DeltaLayoutEngine:
    ...
    def build(
        ...
        width_scale = max(
            0.35,
            min(2.5, bounds.width() / self.BASE_WIDTH),
        )
        ...
        header_h = (
            64.0 * width_scale
            if self._enabled(elements, "header")
            else 0.0
        )
        ...
        sectors_h = (
            76.0 * width_scale
            if self._enabled(elements, "sectors")
            else 0.0
        )
        ...
        content_height = (
            sum(visible_heights)
            + gap * max(0, len(visible_heights) - 1)
        )
        preferred = int(margin * 2 + content_height)

        content_width = max(40.0, bounds.width() - margin * 2)
        outer = QRectF(
            bounds.left() + margin,
            bounds.top() + margin,
            content_width,
        ...
        return DeltaLayout(
            ...
            fastest=fastest,
            sectors=sectors,
            preferred_height=max(90, preferred),
        )
